from dao import PurchaseOrderDAO
from utility import *
from domain import *
from dao import *
from bo import *
...
def menu_details(ch):
    if(ch==1):
        ...
        print("%-20s %-20s %-20s %-20s %-20s %-20s"%("Id" , "Name", "Username", "Mobile Number", "Email-id", "Address"))
        ...
        if(cust==None):
            ...
            return
        salesLead=None
        if(user.get_role().lower()==("store manager")):
...
while(True):
    ...
    if(choice==1):
        ...
        elif(user.get_role().lower()=="store manager" or user.get_role().lower()=="product lead" or user.get_role()=="sales lead"):
            ch = 0
            while(True):
